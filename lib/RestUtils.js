...
exports.errorRes = function(res, msg){
	var msg = msg || '500 ERROR';
	res.statusCode = 500;
	res.end(resMsg(500, msg));
}
exports.errorRes404 = 	function(res, msg){
	var msg = msg || '404 Error';
		res.statusCode = 404;
		res.end(resMsg(404, msg));


}
exports.forbidden = function(res, msg){ //禁止访问函数
		var msg = msg || '403 Forbidden';
		res.statusCode = 403;
		res.end(resMsg(403, msg));
	};
...
var resMsg = function(code, msg){
	var version='';
	fs.readFile(__dirname+'/../package.json', 'utf-8', function(err, data){
		if(!err) version = JSON.parse(data).version;
	})
	return function(code, msg){
		var str = '<html><head><title>'+code+' '+msg+'</title></head><body bgcolor="white"><center><h1>'+code+' '+msg+'</h1></center><hr><center><p>rrestjs / '+version+' <br/> high performace node.js roa & restful framework</p></center></body></html>'
		return str;
	}
}();