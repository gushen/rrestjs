var	parse = require('url').parse, //读取配置，加速加载
	...
	bridge = function(req, res, callback){//核心入口处
		...
		else if(req.path[0] === staticStr && req.method === 'GET'){ //如果是请求静态文件，自动响应静态文件，这里为了效率只能设定一个静态目录
...
if(_restConfig.isCluster){//开启集群则输出以下函数
		...
		_rrest = module.exports = function(callback){
			var callback = callback || function(req, res){};
				cp = ClusterPlus({
					...
					if(cp.isMaster){
						_restConfig.worklistenPort = _restConfig.listenPort;
						_restConfig.listenPort = _restConfig.adminListenPort;
						_restConfig.workobj = _rrest.workobj = cp.workobj;//将工作的子进程号传递给 _rrest 对象
	...
	_rrest.listen = function(server, port){
		...
		if(cp.isMaster){
			...
			createWorker.on('create', function(num){
