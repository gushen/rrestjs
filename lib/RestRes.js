...
RestRes.prototype={
		...
		_clearcookie:function(res, name, options){  //清楚cookie
				var opts = { expires: new Date(1), path: '/' };
				this._cookie(res, name, '', options
				? RestUtils.merge(opts, options)
				: opts);
				return res;
		},
		...
		_render:function(res, view, options, fn){ //输出模版
			var fn = fn||function(){};
			if(tempSet === 'jade') JadeRender(res, view, options, fn); //输出jade模版
			return res;
		},
