...
var fs = require('fs'),
    ...
    RestRes  = module.exports = function(){
		...
		this._restResMethod = ['cache', 'send', 'sendjson', 'sendjsonp', 'sendfile', 'file', 'contenttype', 'download', 'r404', 'attachment', 'set', 'get', 'clearcookie', 'cookie', 'cookiep3p', 'redirect', 'render', 'favicon'];
...
RestRes.prototype={
		...
		_redirect:function(res, url){ //跳转
			  ...
			  res.end(msg.tipsmsg.resRedirect + url);
			  return res;
		},
		_render:function(res, view, ispage, options, fn){ //输出模版
			if(arguments.length === 2){//如果只2个参数，view必传
				templateRender(res, view, '', {}, function(){});
			}
			else if(arguments.length === 3){//传3个参数，则可能是options和fn
				if('function' === typeof ispage) templateRender(res, view, '', {}, ispage);
				else templateRender(res, view, '', ispage, function(){});
			}					
			else if(arguments.length === 4){//如果传递4个参数，则可能options后者fn没穿
				if('function' === typeof options) templateRender(res, view, '', ispage, options);
				else templateRender(res, view, ispage, options, function(){});
			}
			else templateRender(res, view, ispage||'',  options||{}, fn||function(){}); //如果传递参数是4个，表示此页面无需分页，则ispage设置为空
			return res;
		},
...
var templateRender = function(){//选择模版输出
	if(tempSet === 'jade') return JadeRender; //输出jade模版
	else if(tempSet === 'ejs') return ejsRender;//输出ejs模版
	else return function(res){//否则模版设置错误
