var config = _restConfig,
	...
	ending = function(){
		restlog.debug('rrestjs process '+process.pid+' Config file checking complete, errors:'+error+', warns:'+warn);
	},
	...
	//common
		...
		charset:function(val){
			var ary = ['utf-8'];
			if(!checkinarray(val, ary)) return errfn('charset must be in ['+ary.join(',')+'] tolowercase');
			return true;
		},
		autoStatic:function(val){
			if(!checkpath(val)) return errfn('autoStatic must not be a path, for example: /static');
			return true;
		},
...
//static
		...
		staticLv2MaxAge:function(val){
			if(!checknumber(val)) return errfn('autoCreateFolders must be a number, for example:1000*60*60');
			return true;
		},
//session
...
//template
		tempSet:function(val){
			var ary = ['jade','ejs'];
			if(!checkinarray(val, ary)) return errfn('tempSet must be in ['+ary.join(',')+'] tolowercase');
			return true;
		},		
...
//mongodb 
		...
		MongodbCennectTimeout:function(val){
			if(!checknumber(val)) return errfn('MongodbCennectTimeout must be a number, for example: 1000*30 = 30 minutes');
			return true;
		},	
...
function checkip(val){
	if(!(/^(1[0-9]{0,2}|2[1-5]{0,2}).(0|1[0-9]{0,2}|2[1-5]{0,2}).(0|1[0-9]{0,2}|2[1-5]{0,2}).(0|1[0-9]{0,2}|2[1-5]{0,2})$/.test(val))) return false
	return true;
}
