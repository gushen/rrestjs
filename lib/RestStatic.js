...
var acceptEncoding = req.headers['accept-encoding'], //获取客户端是否支持gzip
	...
	canZlib = RestStatic.zlibArray.some(function(value){//判断文件类型是否在gizp输出数组中
		...
		fs.readFile(filepath, function(err, data){
				if(err){ //如果错误，则输出404
					RestUtils.errorRes404(res, 'Sorry not found!');
					return cb(err);
				}
...
RestStatic.send = module.exports = function(res, filepath, cb){ //核心函数，主入口
	var option = RestStatic.option,
		...
		mtime;
	if(option.getOnly && 'GET' != req.method){
		var errstr = 'Sorry this source get method only';
	...
	if (mtime = mBuffer[filepath]){//如果找到2级缓存	
		...
		RestStatic.sendfiles(req, res, filepath, stype, cb); //输出文件
			...
			fs.stat(filepath, function(err, stats){
				  if (err){
					  RestUtils.errorRes404(res, 'Sorry not found file!');
					  return cb(err);
				  }
				  else if (stats.isDirectory()){
					  RestUtils.errorRes404(res, 'Sorry not found file!');
					  return cb('Could not send a directory!');
				  }
...
RestStatic.mBuffer = function(key, mtime){
	...
	if(!mBuffer[key]){
		if(++mBuffer.length>10000) RestStatic.clearmBuffer();//如果mBuffer不存在，并且长度已经超过1000了，清空2级缓存Buffer
		}	
	mBuffer[key] = mtime;
};
RestStatic.clearmBuffer = function(){
