	...
	staticObj = {
		...
		autoStaticFn:function(req, res){
			var filepath = staticFolder+'/'+req.path.slice(1).join('/'), //拼装静态文件绝对地址
				mType = staticObj.getMimeType(filepath);//根据绝对地址获取后缀名
			if(mType==='css'){//如果是css文件则去生成less.前缀的css文件名
			...
			fs.readFile(filepath, 'utf-8',function(err, data){//根据请求读取目录
				...
				less.render(data, function (err, css) {//开始编译less文件
					...
					var po = filepath.lastIndexOf('/')+1,
						...
						lesspath = filepath.replace(path, 'less.'+path);//根据元有文件路径拼装成带less.为前缀的less css文件
...
if(!staticParse){//如果没有开启静态文件整合功能则直接自动输出静态文件
	...
	module.exports = function(req, res){
		...
		if(!parsePath) return staticObj.autoStaticFn(req, res); //开启功能但未使用此方法，则直接自动输出，例如请求是图片或者txt
			...
			if(pobj && (Date.now() - staticParseCacheTime < pobj.timestamp)){//如果有缓存，并且缓存未超时,正常输出缓存文件			
				res.sendfile(pobj.filepath, function(err){
					if(err){
						outerror(msg.parse(msg.errmsg.parseCacheNotFound+pobj.filepath, err));
