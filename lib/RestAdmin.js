var outerror = require('./Outerror'),
	...
	startTime = RestUtils.fdate('y-m-d h:m:s'),
	lastnum = _restConfig.logPath.lastIndexOf('/'),
	logpath = _restConfig.baseDir+_restConfig.logPath.slice(0, lastnum),
	adminobj = {
		delete:function(req, res){
			if(req.method !== 'POST') return RestUtils.forbidden(res, 'Must has post method！');  
			var  result = _restConfig.workobj.some(function(value){return value == req.postparam.pid})
			if(result){
		...
		index:function(req, res){
			...
			fs.readFile(__dirname+'/admin/admin.html', 'utf-8', function (err, htmldata) {
				 ...
				 fs.readdir(logpath, function(err, files){
					...
					htmldata = htmldata.replace('$listenport', _restConfig.worklistenPort);
					htmldata = htmldata.replace('$pid', _restConfig.workobj.join(', '));
