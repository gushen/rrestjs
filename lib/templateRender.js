...
tempobj.tempHeader =  function(res, html){//设置jadeer模版响应头
	var size = Buffer.byteLength(html, charset);
	res.setHeader('Content-Type', 'text/html; charset='+charset);
	res.setHeader('Content-Length', size);
	return res;
};
tempobj.renderFile = function(view, viewkey, fn, callback){
...
tempobj.genCache = function(err, view, viewkey, html){//生成缓存映射表
	...
	fs.writeFile(CacheFolder+'/'+filename, html, charset, function(err){//写入文件
		...
		if(err){//如果出错，则删除缓存映射表的这个key，记录错误日志
			...
			outerror('Write template_cache error: ' + view);
		}
	});
};
