...
jadeobj.genCache = function(err, view, html){//生成缓存映射表
	if(err){
		if(jadeobj.BufferObj[view]) jadeobj.BufferObj[view].key = 1;
		return 
...
jadeobj.sendCache = function(res, view, options, fn){ //响应缓存
	var filename = CacheFolder + '/' +jadeobj.BufferObj[view].data; //拼装jade缓存文件地址
	fs.readFile(filename, charset, function(err, data){
		if(err){ //如果未找到缓存文件
			jadeobj.renderFile(res, view, options, fn, jadeobj.genCache);//如果未找到文件则去生成文件
			return;
		}
...
if(jadeobj.isCache){//如果开启了jade html缓存
	...
	module.exports = function(res, view, options, fn){
			...
			else jadeobj.sendCache(res, view, options, fn); //否则输出缓存
	};
}
